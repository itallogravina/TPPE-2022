...
function getSomatoriaDeducao(previdencia, dependentes, pensao) {
  const sum =
    getTotalDeducao(previdencia) +
    getTotalDependentes(dependentes) +
    getTotalPensao(pensao);
...
function getBasedeCalculo(rend, previdencia, dependentes, pensao) {
  const sum =
    getTotalRendimento(rend) -
    getSomatoriaDeducao(previdencia, dependentes, pensao);
  var fixedNum = parseFloat(sum.toFixed(2));
...
function Imposto(p) {
  ...
  if (p <= i) {
    ...
    const temp =
      parseFloat(((p - i - 922.67 - 924.4) * 0.225).toFixed(4)) +
      69.2003 +
      138.66;
  ...
  if (p > 4664.68) {
    const temp =
      parseFloat(((p - i - 922.67 - 924.4 - 913.63) * 0.275).toFixed(4)) +
      69.2003 +
      138.66 +
      205.5667;
...
function CalculandoAlicota(rend, previdencia, dependentes, pensao) {
  ...
  let b = parseFloat(((a.imposto / getTotalRendimento(rend)) * 100).toFixed(2));
  if (isNaN(b)) {
    b = 0;
  }
